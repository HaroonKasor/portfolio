...
Dark mode selector: `html[data-theme="dark"]` (next-themes `attribute="data-theme"`, `defaultTheme="light"` (light by default; system preference is not applied until the visitor toggles)).
...
- projects (03 WORK / ผลงาน): "Work I actually shipped" / "งานที่ผมลงมือทำจริง" — 4 cards, order & data:
  ...
  3. `membership-booking` 03 badge SPRING BOOT + REACT cover #33BF99→#0D7366 tags [Spring Boot, React, SQL Server] · EN "Membership & Course Booking" / TH "ระบบสมาชิกและจองคอร์สอบรม" · EN "Training-course booking for a corporate safety-training client: VIP membership, points, coupons and invoice PDFs with Thai VAT and withholding tax. I work across the Spring Boot API, the React back-office and the customer site." TH "ระบบจองคอร์สอบรมของลูกค้าด้านฝึกอบรมความปลอดภัย: สมาชิก VIP คะแนนสะสม คูปอง และใบแจ้งหนี้ PDF พร้อม VAT และภาษีหัก ณ ที่จ่าย ผมทำงานทั้ง Spring Boot API, back-office ด้วย React และเว็บลูกค้า" (rewritten from repo evidence 2026-08-30)
  4. `obec-vr-learning` 04 badge SPRING BOOT + UNITY cover #8C73FF→#4D33B3 (image `/images/obec-vr-learning.webp`) tags [Spring Boot, React, Unity, MySQL] · EN/TH title "OBEC MyLearningTime" · EN "A VR learning platform for OBEC, Thailand’s Office of the Basic Education Commission: a Spring Boot API with 129 documented endpoints, a React portal, and 18 Unity VR science games wired to record every play session." TH "แพลตฟอร์มเรียนรู้ผ่าน VR ของ สพฐ. (OBEC): Spring Boot API ที่มีเอกสาร 129 endpoints, portal ด้วย React และเกม VR วิทยาศาสตร์ 18 เกมบน Unity ที่เชื่อมต่อให้บันทึกผลการเล่นทุกครั้ง" (merged from the former `gov-elearning` + `vr-science-games` cards, 2026-08-30)
